(ns merkle-lsm.compaction
  "Compaction POLICY over `merkle-lsm.core`: range compaction scheduler,
   L0 sub-levels, leases, safe-epoch pinning, garbage collection.

   The MECHANISM (merge, retention, run emission) lives in
   `merkle-lsm.core` (`compact-runs`, `compact-runs-partitioned`,
   `compact-run-readers-streaming`, `compact-chunk`/`compact-chunks`).
   This namespace decides WHICH runs to merge, WHEN, under WHICH lease, and
   WHICH blocks are then unreachable -- it calls into core for the merge
   itself (`compact-runs-partitioned`) and for reachability
   (`linked-cids`).
   ...
   Extracted verbatim from kotobase-peer @780b2216 (ADR-2607201600 M4)."
